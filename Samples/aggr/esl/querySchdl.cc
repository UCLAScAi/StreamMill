...
namespace ESL {
	...
	int querySchdl::run()
	{
		...
		qsLog = (char*)malloc(11);
		sprintf(qsLog, "./QS.log");

		stdoutLog = freopen(qsLog, "w", stdout);
		stderrLog = freopen(qsLog, "a", stderr);  


		//Hamid: added for test
		querySchdl::verbose = 1; 
		...
		querySchdl::SMLOG(10, "starting to log. level is %i\n", SMLOG_LEVEL );
		...
		while (1) 
		{
			...
			// Hamid: I added this if to decrease the effect of busy waiting
			// FIXME: it seems that && opration above must be an ||
			// 7/9/2009
			if (qBuf->empty())
			{
			//	cout << "Hamid: buffer is empty but driver is not null" 
			//		<< " Possible busy waiting problem"
			//		<< endl;
				usleep(1000);//FIXME: you may wanna reduce this value
			}
			
			//if (!(qBuf->empty())) {
			while (!(qBuf->empty()) 
					|| (exp_batch_activate_mode && activated > 0 
						&& activated < num_queries)) 
			{
				if (qBuf->empty()) continue;
				...
				if (rc == 0){
					...
					strcpy(id3, (char*)cdbt.data+sizeof(int)+strlen(id)+1+strlen(id2)+1);		
					qBuf->pop();
			...
			//cout << "run all drivers once" << endl;
			bool no_input = true;
			run_rc rc;
			...
			for (list<Driver*>::iterator itr = drvs->begin(); itr!=drvs->end(); itr++) {  
				...
				if (rc == run_success || rc == run_failure) 
					no_input = false;
				//if (querySchdl::verbose && (*itr)->stateTableChanged) cout << "3 ";
			}
			fflush(stderrLog);
			fflush(stdoutLog);
		...
		run_rc querySchdl::runDriver(Driver* drv) {
			...
			if (drv->isDirty() && use_strategies) {
				...
				if (drv->getStrategy(true) != NULL) drv->getStrategy(true)->update();
				else if (drv->getStrategy() != NULL) drv->getStrategy()->update();
			}

			//if (querySchdl::verbose && drv->stateTableChanged) cout << "5 ";

			if (use_strategies && drv->getStrategy(true) == NULL && drv->getStrategy() == NULL) {
				dm->getMonitor()->updateMaps();
		...
		{
			querySchdl::SMLOG(10, "Entering querySchdl::ADD_QUERY_CMD_CODE_hndl");
			int rc = compile(id);
...
{
	querySchdl::SMLOG(10, "Entering querySchdl::SNAP_SHOT_CMD_CODE_hndl");
			...
			if(ret == 0) {
				...
				if(!handle) {
					...
					for(stmt = 0; stmt < stmts; stmt++) {
						...
						//fflush(stdout);
						int (*snapshot) (bufferMngr*, int, buffer*, 
...
void querySchdl::ADD_DECLARE_CMD_CODE_hndl(bufferMngr* bm, buffer* qBuf, char * id, int code )
{
	querySchdl::SMLOG(10, "Entering querySchdl::ADD_DECLARE_CMD_CODE_hndl");
	compiler *cc = compiler::getInstance();
	Driver *drv=NULL;
	buffer *iob = bm->lookup("_ioBuffer");
	...
	else 
	{
		...
		if (code == ADDED_BUFFER) {
			...
			//check in free pool	    
			for (list<stmt*>::iterator itr = freeStmts.begin(); itr!=freeStmts.end() && !error; itr++) {
				bool found = false;
				...
				if ((*itr)->type == stmt_t_union || (*itr)->type == stmt_tl_union) {
					...
					for (list<stmt*>::iterator itr2 = s1->sub_stmts.begin(); itr2!=s1->sub_stmts.end();itr2++) {
					...
					for (list<stmt*>::iterator itr2 = s1->sub_stmts.begin(); itr2!=s1->sub_stmts.end();itr2++) {
						if((*itr2)->type == stmt_t_union 
								|| ((*itr2)->type == stmt_tl_union)) {
							uStmt* s2 = static_cast<uStmt*> (*itr2);
							int sub_count2 = s2->sub_stmts.size();

							for (list<stmt*>::iterator itr3 = s2->sub_stmts.begin(); itr3!=s2->sub_stmts.end();itr3++) {
					...
					if (strcmp((*itr)->inBufName, id) == 0 || strcmp((*itr)->outBufName, id) == 0) {
						found = true;
					}
			...
			if(!error) {
				...
				for (list<stmt*>::iterator itr3 = toRemove.begin(); itr3!=toRemove.end();itr3++) {
...
{
	querySchdl::SMLOG(10, "Entering querySchdl::ACTIVATE_QUERY_CMD_CODE_hndl");
...
if (!err) {
	...
	while (!err && !(q.empty())) {
		...
		if (s3->type == stmt_coll) {
			...
			//printf("we here %d\n", sub_count);
			if (querySchdl::verbose) {
				cout << "there are " << sub_count << " sub statements in this coll statement." << endl;
			}

			for (list<stmt*>::iterator itr = s1->sub_stmts.begin();
					!err && itr!=s1->sub_stmts.end();itr++) {
				if (querySchdl::verbose) {
				...
				if ((*itr)->type == stmt_t_union || (*itr)->type == stmt_tl_union) {
					...
					int sub_count2 = s2->sub_stmts.size();
					if (querySchdl::verbose) {
						cout << "there are " << sub_count2 << " sub statements in this union statement." << endl;
					}

					for (list<stmt*>::iterator itr2 = s2->sub_stmts.begin();
							!err && itr2!=s2->sub_stmts.end();itr2++) {
						if (querySchdl::verbose) {
			...
			int sub_count = s1->sub_stmts.size();
			if (querySchdl::verbose) {
				cout << "there are " << sub_count << " sub statements in this union statement." << endl;
			}
			...
			int sub_count = s1->sub_stmts.size();
			if (querySchdl::verbose) {
				cout << "there are " << sub_count << " sub statements in this join statement." << endl;
			}
		...
		if (!err && querySchdl::verbose) {
			cout << "add stmt to driver: "
				<< s3->name << ", of type " << s3->type << ", with out buffer " << s3->out->name << endl;
		}
		...
		for (list<stmt*>::iterator itr = freeStmts.begin(); itr!=freeStmts.end(); itr++) {
			...
			for (list<char*>::iterator itr3 = bufs.begin(); !found && itr3!= bufs.end(); itr3++) {
...
if (exp_batch_activate_mode) activated++;
...
{
	querySchdl::SMLOG(10, "Entering querySchdl::DEACTIVATE_QUERY_CMD_CODE_hndl");
	...
	if (s!= NULL && (s->type == stmt_coll)) {
		...
		for (list<stmt*>::iterator itr = s1->sub_stmts.begin(); itr!=s1->sub_stmts.end();itr++) {
			if (querySchdl::verbose) {
				cout << "current sub statement name is " << (*itr)->name << endl;
			...
			if ((*itr)->type == stmt_t_union || (*itr)->type == stmt_tl_union) {
				...
				for (list<stmt*>::iterator itr2 = s2->sub_stmts.begin(); itr2!=s2->sub_stmts.end();itr2++) {
					if (querySchdl::verbose) {
						cout << "current sub statement name is " << (*itr2)->name << endl;
...
{
	querySchdl::SMLOG(10, "Entering querySchdl::DROP_QUERY_CMD_CODE_hndl");
...
{
	querySchdl::SMLOG(10, "Entering querySchdl::DROP_DECLARE_CMD_CODE_hndl");
	buffer *iob = bm->lookup("_ioBuffer");
...
{
	querySchdl::SMLOG(10, "Entering querySchdl::GET_COMPONENTS_hndl");
	buffer *iob = bm->lookup("_ioBuffer");
...
{
	querySchdl::SMLOG(10, "Entering querySchdl::VIEW_COMPONENT_DETAILS_hndl");
	buffer *iob = bm->lookup("_ioBuffer");
	...
	if (drv == NULL) {
		...
		if (stmts -> empty()) {
			...
			bool first = true;
			for (list<b_entry>::iterator itr = sources.begin(); itr != sources.end() && rc != COMMAND_FAILED; itr++) {
				...
				while (!entry_stack.empty() && rc != COMMAND_FAILED) {
					...
					if (pse->entry->b_type == t_sink || pse->entry->b_type == t_merge_sink) {
						//no-op
					} else if (pse->entry->b_type == t_fork || pse->entry->b_type == t_source_fork
							|| pse->entry->b_type == t_merge_fork) {
						...
						for (BufStateTblType::iterator itr2 = bufStateTable->begin(); itr2 != bufStateTable->end(); itr2++) {
							...
							else if (strcmp(itr2->second->inplace, pse->entry->buf->name) == 0) {
								//as a fork dummy, my next is always unique
								b_entry next = (*bufStateTable)[itr2->second->forward.forward_buf];
								se = new stack_item();
						...
						if (next->b_type != t_sink && next->b_type != t_merge_sink 
								&& (next->statement->type == stmt_t_union || next->statement->type == stmt_tl_union
									|| (next->statement->type == stmt_join && next->buf->type == WINBUF))) {
...
void querySchdl::SET_COMPONENT_PRIORITY_hndl(bufferMngr* bm, char * id , char * id2 )
{
	querySchdl::SMLOG(10, "Entering querySchdl::SET_COMPONENT_PRIORITY_hndl");
...
void querySchdl::MOVE_STMT_TO_COMPONENT_hndl(bufferMngr* bm, char * id, char * id2, char* id3)
{
	querySchdl::SMLOG(10, "Entering querySchdl::MOVE_STMT_TO_COMPONENT_hndl");
	buffer *iob = bm->lookup("_ioBuffer");
	...
	if (s == NULL || drv1 == NULL || drv2 == NULL) {
		if (querySchdl::verbose) {
			cout << "stmt or driver not found in move component: "
				<< id << ", " << id2 << ", " << id3 << endl;
		}
		...
		if (dm->dropStmt(s, drv4) != SUCCESS) {
			...
			if (querySchdl::verbose) {
				cout << "unable to add stmt " << s->name << " to driver " << drv2->getId() << endl;
			}
...
{		
	querySchdl::SMLOG(10, "Entering querySchdl::BREAK_COMPONENT_hndl");
	if (querySchdl::verbose) {
...
{		
	querySchdl::SMLOG(10, "Entering querySchdl::JOIN_COMPONENTS_hndl");	
	buffer *iob = bm->lookup("_ioBuffer");
	...
	if (drv1 == NULL || drv2 == NULL) {
		if (querySchdl::verbose) {
			cout << "driver not found in join components: " << id << ", " << id2 << endl;
		}
		...
		if (!remove.empty()) {
			for (list<stmt*>::iterator itr = remove.begin();
					itr!= remove.end() && rc != COMMAND_FAILED; itr++) {
				Driver *drv4=NULL;
			...
			for (list<stmt*>::iterator itr = remove.begin();
				...
				if (dm->addStmt((*itr), drv2) != SUCCESS) {
					if (querySchdl::verbose) cout << "error adding statement "
						<< (*itr)->name << " to driver " << drv2->getId() << endl;
					rc = COMMAND_FAILED;
				}
			}
		}
...
{		
	querySchdl::SMLOG(10, "Entering querySchdl::MONITOR_PERFORMANCE_BUFFER_hndl");	
	Monitor* monitor = dm->getMonitor();
	//if (strlen(id2) == 0) {
...
{
	querySchdl::SMLOG(10, "Entering querySchdl::UNMONITOR_PERFORMANCE_BUFFER_hndl");	
	Monitor* monitor = dm->getMonitor();
...
void querySchdl::SMLOG(int level, const char* Format, ...)
{
return;
	if (level > SMLOG_LEVEL)
		return;
	FILE *fp;
	if((fp=fopen("../SMLOG.log", "w")) == NULL) 
	{
		...
		if (!(handle = dlopen (libName, RTLD_NOW))) {
			...
			for (list<stmt*>::iterator itr = s1->sub_stmts.begin(); itr!=s1->sub_stmts.end();itr++) {
				...
				if((*itr)->type == stmt_t_union || (*itr)->type == stmt_tl_union) {
					...
					if (querySchdl::verbose) {
						cout << "there are " << sub2_count << " sub statements in this union statement." << endl;
					}
					for (list<stmt*>::iterator itr2 = s2->sub_stmts.begin(); itr2!=s2->sub_stmts.end();itr2++) {
						if (querySchdl::verbose) {
							cout << "current sub statement name is " << (*itr2)->name << endl;
						}
						if (!(((*itr2)->func) = (int (*)(bufferMngr*, int, buffer*, hash_map<const char*, void*, hash<const char*>, eqstrTab>*))dlsym(handle, (*itr2)->name))) {
				...
				else if (!(((*itr)->func) = (int (*)(bufferMngr*, int, buffer*, hash_map<const char*, void*, hash<const char*>, eqstrTab>*))dlsym(handle, (*itr)->name))) {
					fprintf(stderr, "dlsym: %s %d\n", dlerror(), __LINE__);
					err = true;
					break;
				}	      
			...
			for (list<stmt*>::iterator itr = s1->sub_stmts.begin(); itr!=s1->sub_stmts.end();itr++) {
				...
				if (!(((*itr)->func) = (int (*)(bufferMngr*, int, buffer*, hash_map<const char*, void*, hash<const char*>, eqstrTab>*))dlsym(handle, (*itr)->name))) {
					fprintf(stderr, "dlsym: %s %d\n", dlerror(), __LINE__);
					err = true;
					break;
				}	      
			...
			if (!((s->func) = (int (*)(bufferMngr*, int, buffer*, hash_map<const char*, void*, hash<const char*>, eqstrTab>*))dlsym(handle, id))) {
				fprintf(stderr, "dlsym: %s %d\n", dlerror(), __LINE__);
				err = true;
			}    
...
bool querySchdl::bufferInUse(char* bname) 
{
	for (list<stmt*>::iterator itr = freeStmts.begin(); itr!=freeStmts.end(); itr++) {
		if ((*itr)->type == stmt_coll) {
			...
			if (strcmp((*itr)->in->name, bname) == 0 || strcmp((*itr)->out->name, bname) == 0) {
				return true;
			}
		} else if ((*itr)->type == stmt_normal) {
			if (strcmp((*itr)->in->name, bname) == 0 || strcmp((*itr)->out->name, bname) == 0) {
...
{
	for (list<stmt*>::iterator itr = freeStmts.begin(); itr!=freeStmts.end(); itr++) {
		if(strcmp((*itr)->name, name) == 0) return true;
	}
	return false;
}
...
int querySchdl::simpleTest() 
{
	...
	if (!((s->func) = (int (*)(bufferMngr*, int, buffer*, hash_map<const char*, void*, hash<const char*>, eqstrTab>*))dlsym(handle, "stream"))) {
		fprintf(stderr, "dlsym: %s %d\n", dlerror(), __LINE__);
		exit(1);
	}
...
int querySchdl::forkTest() 
{
	...
	if (!((s->func) = (int (*)(bufferMngr*, int, buffer*, hash_map<const char*, void*, hash<const char*>, eqstrTab>*))dlsym(handle, "stream2"))) {
		fprintf(stderr, "dlsym: %s %d\n", dlerror(), __LINE__);
		exit(1);
	}
	cout << "add stmt to driver: " << s->name << ", with in buffer: " << s->in->name << ", out buffer " << s->out->name << endl;
	...
	if (!((s->func) = (int (*)(bufferMngr*, int, buffer*, hash_map<const char*, void*, hash<const char*>, eqstrTab>*))dlsym(handle, "stream3"))) {
		fprintf(stderr, "dlsym: %s %d\n", dlerror(), __LINE__);
		exit(1);
	}
	cout << "add stmt to driver: " << s->name << ", with in buffer: " << s->in->name << ", out buffer " << s->out->name << endl;
	...
	if (!((s->func) = (int (*)(bufferMngr*, int, buffer*, hash_map<const char*, void*, hash<const char*>, eqstrTab>*))dlsym(handle, "stream4"))) {
		fprintf(stderr, "dlsym: %s %d\n", dlerror(), __LINE__);
		exit(1);
	}
	cout << "add stmt to driver: " << s->name << ", with in buffer: " << s->in->name << ", out buffer " << s->out->name << endl;
...
int querySchdl::tUnionTest() 
{
	...
	if (s->type == stmt_t_union || s->type == stmt_tl_union) {
		...
		if (!((s->func) = (int (*)(bufferMngr*, int, buffer*, hash_map<const char*, void*, hash<const char*>, eqstrTab>*))dlsym(handle, sname))) {
			fprintf(stderr, "dlsym: %s %d\n", dlerror(), __LINE__);
			exit(1);
		}    
	}

	cout << "add stmt to driver: " << s->name << ", of type " << s->type << ", with out buffer " << s->out->name << endl;
	dm->addStmt(s, drv);
	drv->setPriority(5);
	drv->printStateTable();
...
int querySchdl::tlUnionTest() 
{
	...
	if (s->type == stmt_t_union || s->type == stmt_tl_union) {
		...
		if (!((s->func) = (int (*)(bufferMngr*, int, buffer*, hash_map<const char*, void*, hash<const char*>, eqstrTab>*))dlsym(handle, "test"))) {
			fprintf(stderr, "dlsym: %s %d\n", dlerror(), __LINE__);
			exit(1);
		}    
	}
...
int querySchdl::splitTest() 
{
	...
	if (!((s->func) = (int (*)(bufferMngr*, int, buffer*, hash_map<const char*, void*, hash<const char*>, eqstrTab>*))dlsym(handle, "stream5"))) {
		fprintf(stderr, "dlsym: %s %d\n", dlerror(), __LINE__);
		exit(1);
	}
	cout << "add stmt to driver: " << s->name << ", with in buffer: " << s->in->name << ", out buffer " << s->out->name << endl;
