...
//FIXME: Hamid I have to added it only to have access to logging function
...
windowBuf:: windowBuf(const char* name_in, unsigned int size_in,
	...
	querySchdl::SMLOG(10, "Entering windowBuf:: windowBuf");	
  strcpy(name, name_in);
  mode = win_memory;
  wtype = wtype_in;
  size = size_in;
...
int windowBuf::put(DBT* key, DBT* data){
	querySchdl::SMLOG(10, "Entering windowBuf::put");	
...
int windowBuf::pop(){
	querySchdl::SMLOG(10, "Entering windowBuf::pop");	
...
int windowBuf::getExpired(DBT* key, DBT* data){
	querySchdl::SMLOG(10, "Entering windowBuf::getExpired");	
...
int windowBuf::print(){
	querySchdl::SMLOG(10, "Entering windowBuf::print");	
  timestamp ts;
  DBT key, data;
  char keydata[MAX_STR_LEN], datadata[MAX_STR_LEN];
...
void windowBuf::getTimestamp(timestamp *atime) {
	querySchdl::SMLOG(10, "Entering windowBuf::getTimestamp");	
...
int windowBuf::updateTupleID(timestamp *ts){
	querySchdl::SMLOG(10, "Entering windowBuf::updateTupleID");	
