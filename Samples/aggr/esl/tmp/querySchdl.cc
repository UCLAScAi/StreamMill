...
int querySchdl::run(){
  ...
  while (1) {
    ...
    //if(qBuf->empty() && (drvs == NULL || drvs->size() == 0)) {
    ...
    //if (!(qBuf->empty())) {
    while (!(qBuf->empty()) || (exp_batch_activate_mode && activated > 0 && activated < num_queries)) {
	...
	/* now moved to the above function compile()
	void *handle;
	compiler *cc = compiler::getInstance();
	bool err = false;
	...
	if (c_rc == 0) {
	  ...
	  if (!(handle = dlopen (libName, RTLD_NOW))) {
		...
		cout << "current sub statement name is " << (*itr)->name << endl;
	      ...
	      if (!((void*)((*itr)->func) = dlsym(handle, (*itr)->name))) {
	    ...
	    if (!((void*)(s->func) = dlsym(handle, id))) {
	      fprintf(stderr, "dlsym: %s\n", dlerror());
	      err = true;
	    }    
	...
	*/
	...
	if (!err) {
	  ...
	  while (!err && !(q.empty())) {
	    ...
	    if (s3->type == stmt_t_union || s3->type == stmt_tl_union) {
	    ...
	    if (!err && dm->addStmt(s3, drv) == SUCCESS) {
	      //drv->setPriority(5);
	      if (querySchdl::verbose) {
	...
	if (s!= NULL && (s->type == stmt_t_union || s->type == stmt_tl_union)) {
	  uStmt* s1 = static_cast<uStmt*> (s);
	  int sub_count = s1->sub_stmts.size();
	  if (querySchdl::verbose) {
	    cout << "there are " << sub_count << " sub statements in this union statement." << endl;
	...
	iob->put(rc);	
      ...
      if (querySchdl::verbose) {
	cout << "end processing one command" << endl;
      }
    }
    ...
    for (list<Driver*>::iterator itr = drvs->begin(); itr!=drvs->end(); itr++) {  
      rc = runDriver(*itr);
      if (rc == run_success || rc == run_failure) no_input = false;
