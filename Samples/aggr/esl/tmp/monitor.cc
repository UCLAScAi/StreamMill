...
stmt_rc Monitor::exeStmt(stmt* s, Driver* drv) {
  ...
  if (!collectStats) {
    ...
    //this is used in modes when stats should not be collected, such as when we need to avoid overhead when testing certain performances
    int rc = (int) (s->exe());
	...
	double totalDiff = timeval_subtract(aggregated_throughput_endtime, aggregated_throughput_starttime);
	cout << "finished processing " << aggregated_throughput_desired_total << " tuples." << endl;
