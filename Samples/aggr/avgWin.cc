...
int _adl_statement_22()
{
   ...
   struct gb_status_19 {
      struct myavg_online_window_status *myavg_online_window_0;
   };
   ...
   while (rc==0) {
      ...
      while (index_19>=0 && index_19 < 2) {
         switch(index_19) {
               ...
               if (terminating_19 == 0) {
                  ...
                  if (rc==0) {
                     ...
                     if (rc == DB_NOTFOUND) {//blah
                        ...
                        gbstatus_19->myavg_online_window_0 = (struct myavg_online_window_status*)malloc(sizeof(struct myavg_online_window_status));
                        gbstatus_19->myavg_online_window_0->win = 0;
                        gbstatus_19->myavg_online_window_0->win = new winbuf(17, 4, 4, _ADL_WIN_ROW);
                        gbstatus_19->myavg_online_window_0->last_out = 0;
                        gbstatus_19->myavg_online_window_0->iterate = false;
                        gbstatus_19->myavg_online_window_0->init = true;
                        setModelId("");
                        memcpy(_databuf+0, &(Q_0002_19_18.a_0), sizeof(int));
                        windata.data = _databuf;
                        windata.size = 4;
                        gbstatus_19->myavg_online_window_0->win->updateTupleID();
                        gbstatus_19->myavg_online_window_0->win->put(&windata);
                        if((!(1 <= 1 || ((gbstatus_19->myavg_online_window_0->win->getTupleID() == 0 && gbstatus_19->myavg_online_window_0->last_out != 0) 
                        ||((((int)gbstatus_19->myavg_online_window_0->win->getTupleID()) >= gbstatus_19->myavg_online_window_0->last_out + 1)))))) {
                           slide_out = 0;
                           //printf("Here no output %d 1 %d\n", gbstatus_19->myavg_online_window_0->last_out, gbstatus_19->myavg_online_window_0->win->getTupleID());fflush(stdout);
                        } else {
                           slide_out = 1;
                           //printf("Here YES output %d 1 %d\n", gbstatus_19->myavg_online_window_0->last_out, gbstatus_19->myavg_online_window_0->win->getTupleID());fflush(stdout);
                           gbstatus_19->myavg_online_window_0->last_out = gbstatus_19->myavg_online_window_0->last_out + 1;
                           while(gbstatus_19->myavg_online_window_0->last_out < (gbstatus_19->myavg_online_window_0->win->getTupleID() - 1) && gbstatus_19->myavg_online_window_0->win->getTupleID() > 0) {
                              if(1 == 1) {
                                 gbstatus_19->myavg_online_window_0->last_out = gbstatus_19->myavg_online_window_0->win->getTupleID();
                              }
                              else {
                                 gbstatus_19->myavg_online_window_0->last_out = gbstatus_19->myavg_online_window_0->last_out + 1;
                              }
                        ...
                        myavg_online_window_init(gbstatus_19->myavg_online_window_0, Q_0002_19_18.a_0, _rec_id+1, 1, NULL, inMemTables, NULL, 0, getModelId());
                        ...
                        while (gbstatus_19->myavg_online_window_0->win->hasExpired()){
                           gbstatus_19->myavg_online_window_0->win->getExpired(&windata);
                           gbstatus_19->myavg_online_window_0->win->pop();
                        }
                        myavg_online_window_init(gbstatus_19->myavg_online_window_0, Q_0002_19_18.a_0, _rec_id+1, 0, NULL, inMemTables, NULL, 0, getModelId());
                     } else adlabort(rc, "hash->get()");
                  } else if (rc == DB_NOTFOUND) {
                     terminating_19 = 1;
                  }
            ...
            case 1:
            {
               memset(&key, 0, sizeof(key));
               memset(&data, 0, sizeof(data));
               rc = gbstatus_19->myavg_online_window_0->retc->c_get(gbstatus_19->myavg_online_window_0->retc, &key, &data, (gbstatus_19->myavg_online_window_0->retc_first_entry)? DB_FIRST:DB_NEXT);
               if (rc == 0) {
                  gbstatus_19->myavg_online_window_0->retc_first_entry = 0;
                  memcpy(&(Q_0003_17_16.a_0), (char*)data.data+0, sizeof(double));
                  //printf("Retrieved Q_0003_17_16.a_0 = %f\n", Q_0003_17_16.a_0);
                  //fflush(stdout);
                  if ((rc = gbstatus_19->myavg_online_window_0->retc->c_del(gbstatus_19->myavg_online_window_0->retc, 0)) != 0) {
                     adlabort(rc, "IM_RELC->c_del() or DBC->c_del()");
                  }
               } else if (rc == DB_NOTFOUND) {
                  gbstatus_19->myavg_online_window_0->retc_first_entry = 1;
               } else adlabort(rc, "DBC->c_get()");
               first_entry_19 = 0;
            }
            break;
         ...
         if (rc == DB_NOTFOUND) {
            ...
            if (terminating_19 == 1 && index_19 == 0) {
               if (gbstatus_19->myavg_online_window_0->retc && (rc = gbstatus_19->myavg_online_window_0->retc->c_close(gbstatus_19->myavg_online_window_0->retc)) != 0) {
                  adlabort(rc, "IM_RELC->c_close() or DBC->c_close()");
               }
               sprintf(_adl_dbname, "._%d_ret", gbstatus_19->myavg_online_window_0);
               
               if (gbstatus_19->myavg_online_window_0->ret && ((rc = gbstatus_19->myavg_online_window_0->ret->close(gbstatus_19->myavg_online_window_0->ret, 0)) != 0)) {
                  adlabort(rc, "DB->close()");
               }
               gbstatus_19->myavg_online_window_0->ret = NULL;
               (void)unlink(_adl_dbname);
               rc = DB_NOTFOUND;
            }
         }
      ...
      else 
      {
         ...
         do {
            ...
            rc = hash_get(19, _rec_id, allkey, 4, (char**)&gbstatus_19);
            if (rc==0) {
               if(gbstatus_19->myavg_online_window_0) {
                  if(gbstatus_19->myavg_online_window_0->win) {
                     delete(gbstatus_19->myavg_online_window_0->win);
                     gbstatus_19->myavg_online_window_0->win = 0;
                  }
                  free(gbstatus_19->myavg_online_window_0);
               }
               //printf("freeing 19\n");
               free(gbstatus_19);
            }
