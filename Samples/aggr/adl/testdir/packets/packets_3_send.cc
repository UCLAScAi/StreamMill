...
#define TCP_PORT 5620
#define UDP_PORT 5621
#define SF_PORT 5622
#define TIMESTAMP_FORMAT "%I:%M:%S %p %D"  //"%d %b %y %H:%M:%S"

//#define TCP_COUNT 2873589
#define TCP_COUNT 514607
...
typedef struct tcp_type
{
  ...
  void print() 
  {
    cout << "tcp3: " << a_time.tv_sec << "." << a_time.tv_usec << "," << from_ip << "," << to_ip << "," << s_port << "," << d_port << "," << num_bytes << endl;
  }
} tcp_t;
 ...
 if(send_sf) {
  ...
  while (1) {
	...
	if (debug) {
	  ...
	  cout << "sent tcp3 data late for more than 3 sec, delay is: " << SLOW_DOWN*tcps[next_tcp_count].a_time.tv_sec - (tv.tv_sec - start_tv.tv_sec)  << " sec."<< endl;
	}
      }
	...
	if (debug) {
	  //cout << "sent tcp3, values ";
	  //tcps[id].print();
	  cout << "sent tcp3 #" << id << ", buffer: " << buf << endl;
	}
      }
    }
    cout.flush();
