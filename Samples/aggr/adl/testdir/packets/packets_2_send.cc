...
#define TCP_PORT 5610
#define UDP_PORT 5611
#define SF_PORT 5612
#define TIMESTAMP_FORMAT "%I:%M:%S %p %D"  //"%d %b %y %H:%M:%S"

//#define TCP_COUNT 2661931
#define TCP_COUNT 334027
...
typedef struct tcp_type
{
  ...
  void print() 
  {
    cout << "tcp2: " << a_time.tv_sec << "." << a_time.tv_usec << "," << from_ip << "," << to_ip << "," << s_port << "," << d_port << "," << num_bytes << endl;
  }
} tcp_t;
...
typedef struct udp_type
{
  ...
  void print() 
  {
    cout << "udp2: " << a_time.tv_sec << "." << a_time.tv_usec << "," << from_ip << "," << to_ip << "," << s_port << "," << d_port << endl;
  }
} udp_t;
...
typedef struct sf_type
{
  ...
  void print() 
  {
    cout << "sin/fin2: " << a_time.tv_sec << "." << a_time.tv_usec << "," << from_ip << "," << to_ip << "," << s_port << "," << d_port << "," << flags << endl;
  }
} sf_t;
...
int main ( int argc, char** argv )
{
  ...
  if (!loaded) {
    ...
    ifstream in2("./udp2.txt");
    
    if(!in2) {
      cout << "file not found: udp2.txt" << endl;
      return 2;
    }
    ...
    ifstream in3("./sf2.txt");
    
    if(!in3) {
      cout << "file not found: sf2.txt" << endl;
      return 2;
    }
 ...
 if(send_sf) {
  ...
  while (1) {
	...
	if (debug) {
	  ...
	  cout << "sent tcp2 data late for more than 3 sec, delay is: " << SLOW_DOWN*tcps[next_tcp_count].a_time.tv_sec - (tv.tv_sec - start_tv.tv_sec)  << " sec."<< endl;
	}
      }
	...
	if (debug) {
	  //cout << "sent tcp2, values ";
	  //tcps[id].print();
	  cout << "sent tcp2 #" << id << ", buffer: " << buf << endl;
	}
      }
    }
    cout.flush();
