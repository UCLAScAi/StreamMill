...
#define TCP_PORT 5630
#define UDP_PORT 5631
#define SF_PORT 5632
#define TIMESTAMP_FORMAT "%I:%M:%S %p %D"  //"%d %b %y %H:%M:%S"

//#define TCP_COUNT 3862336
#define TCP_COUNT 689934
...
typedef struct tcp_type
{
  ...
  void print() 
  {
    cout << "tcp4: " << a_time.tv_sec << "." << a_time.tv_usec << "," << from_ip << "," << to_ip << "," << s_port << "," << d_port << "," << num_bytes << endl;
  }
} tcp_t;
...
int main ( int argc, char** argv )
{
  ...
  if (!loaded) {
    ...
    cout << "loading tcp4 data files..." << endl;
 ...
 if(send_sf) {
  ...
  while (1) {
    ...
    gettimeofday(&tv, &tz);
    while (send_tcp && 
	   (tcps[next_tcp_count].a_time.tv_sec < SLOW_DOWN*(tv.tv_sec - start_tv.tv_sec)
	    || (tcps[next_tcp_count].a_time.tv_sec == SLOW_DOWN*(tv.tv_sec - start_tv.tv_sec) && (tcps[next_tcp_count].a_time.tv_usec < SLOW_DOWN*(tv.tv_usec - start_tv.tv_usec))))) {
      if (tcps[next_tcp_count].a_time.tv_sec - SLOW_DOWN*(tv.tv_sec - start_tv.tv_sec) > 3) {
	if (debug) {
	  cout << "next sec is: " << tcps[next_tcp_count].a_time.tv_sec << ", diff in sec between now and start is " << SLOW_DOWN*(tv.tv_sec - start_tv.tv_sec) << endl;
	  cout << "next usec is: " << tcps[next_tcp_count].a_time.tv_usec << ", diff in usec between now and start is " << SLOW_DOWN*(tv.tv_usec - start_tv.tv_usec) << endl;
	  cout << "sent tcp4 data late for more than 3 sec: " << (SLOW_DOWN*(tv.tv_sec - start_tv.tv_sec) - tcps[next_tcp_count].a_time.tv_sec) << " sec."<< endl;
	}
      }
