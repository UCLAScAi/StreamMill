...
LOCAL int DelayArg (int);
LOCAL VOID DbugExit (char *);
LOCAL long *DbugMalloc (int);
...
#if 0                           //not sure why these need to be imported
...
char ***_sframep_;
{
    ...
    *_sframep_ = framep;
	...
	(VOID) fprintf (_db_pfp_, PROF_EFMT , 0 /*Clock ()*/, func);
#if AUTOS_REVERSE
...
int *_slevel_;
{
	...
	(VOID) fprintf (_db_fp_, ERR_MISSING_RETURN, _db_process_, func);
      ...
      (VOID) fprintf (_db_pfp_, PROF_XFMT, 0 /*Clock()*/, func);
    } else if (DoTrace ()) {
	DoPrefix (_line_);
	Indent (stack -> level);
	(VOID) fprintf (_db_fp_, "<%s\n", func);
