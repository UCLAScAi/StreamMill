...
namespace ESL {
	...
	Monitor::Monitor()
	{
		...
		SMLog::SMLOG(10, "Entering Monitor::Monitor");	
	...
	{
		SMLog::SMLOG(10, "Entering Monitor::exeStmt");	
		if (s == NULL || !(s->valid)) 
			return s_failure; //this should not have happened.
		...
		if (!collectStats) {
			...
			SMLog::SMLOG(12, "\t Monitor::exeStmt calling nstmt->exe() 1");	
			int rc = (int) (s->exe());
		...
		SMLog::SMLOG(12, "\t Monitor::exeStmt calling nstmt->exe() 2");	
		int rc = (int) (s->exe());
		...
		if (monitor_exp_mode) {
			...
			if (strcmp(s->name, "a131_179_64_67_982067") == 0 || 
				strcmp(s->name, "a131_179_64_67_984652") == 0 || 
				strcmp(s->name, "a131_179_64_67_432364") == 0) {
				int testval = 65635;
				for (int l = 0; l < 50000; l++) {
					testval*testval/(testval-4);
				}
			} else if (strcmp(s->name, "a131_179_64_67_852489") == 0 || 
						strcmp(s->name, "a131_179_64_67_835152") == 0 || 
						strcmp(s->name, "a131_179_64_67_384858") == 0 || 
						strcmp(s->name, "a131_179_64_67_91059") == 0) {
			...
			if (s_entry != NULL && (s_entry->input_tuple_size == 0 || (s_entry->output_tuple_size == 0 && rc > 0 && !s->monitor_sink_stat))) {
				...
				if (s_entry->input_tuple_size == 0) {
					...
					if (querySchdl::verbose) 
						cout << "input tuple size is set to " << s_entry->input_tuple_size << endl;
				}
			}
		...
		if (s_entry != NULL && s_entry->total_input.on && s_entry->total_input.value > 0 && (s_entry->total_input.value % NUM_TUPLES_TO_MONITOR_BEFORE_REST) == 0) {
			...
			if (s_entry->total_input.value > 0 && 
					(s_entry->total_input.value % (10*NUM_TUPLES_TO_MONITOR_BEFORE_REST)) == 0) {
	...
	void Monitor::updateLastInTurn(stmt* s) {
		...
		//SMLog::SMLOG(10, "Entering Monitor::updateLastInTurn");	
...
void Monitor::updateWakeup() {
		SMLog::SMLOG(10, "Entering Monitor::updateWakeup");	
	if (querySchdl::verbose) {
	...
	for (StmtStatMapType::iterator itr = stmtStats->begin(); itr != stmtStats->end(); ++itr) {
		...
		if (!(itr->second->total_input.on)) {
			if (itr->second->total_input.wakeup_time.tv_sec != 0 && 
				itr->second->total_input.wakeup_time.tv_sec < (tv.tv_sec+1)) 
			{ //roughly is enough
		...
		if (!(itr->second->win_total_input.on)) {
			if (itr->second->win_total_input.wakeup_time.tv_sec != 0 && 
				itr->second->win_total_input.wakeup_time.tv_sec < (tv.tv_sec+1)) 
			{ //roughly is enough
		...
		if (!(itr->second->win_total_output.on)) {
			if (itr->second->win_total_output.wakeup_time.tv_sec != 0 && 
				itr->second->win_total_output.wakeup_time.tv_sec < (tv.tv_sec+1)) 
			{ //roughly is enough
		...
		if (!(itr->second->win_total_p_time.on)) {
			if (itr->second->win_total_p_time.wakeup_time.tv_sec != 0 && 
				itr->second->win_total_p_time.wakeup_time.tv_sec < (tv.tv_sec+1)) { //roughly is enough
				itr->second->win_total_p_time.on = true;
			}
		}

		if (!(itr->second->win_cumul_input_buf_size.on)) {
			if (itr->second->win_cumul_input_buf_size.wakeup_time.tv_sec != 0 && 
				itr->second->win_cumul_input_buf_size.wakeup_time.tv_sec < (tv.tv_sec+1)) { //roughly is enough
...
void Monitor::updateMaps() {
		SMLog::SMLOG(10, "Entering Monitor::updateMaps");	
	DrvMgr* dm = DrvMgr::getInstance();
	if (querySchdl::verbose) {
...
{
	SMLog::SMLOG(10, "Entering Monitor::printStateTables");	
	DrvMgr* dm = DrvMgr::getInstance();
...
{
	SMLog::SMLOG(10, "Entering Monitor::addSendStat");	
...
{
	SMLog::SMLOG(10, "Entering Monitor::removeSendStat");	
	perf_stat_info pair;
