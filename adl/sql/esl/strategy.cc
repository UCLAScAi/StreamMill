...
namespace ESL {
	...
	WeightProfile::WeightProfile(Driver* drv, int roundTotal, quota_mode mode) : roundTotal(roundTotal),mode(mode),stmtWithDeadline(false)  {
		SMLog::SMLOG(10, "Entering WeightProfile::WeightProfile");	
		this->drv = drv;
		totalSourceStmt = 0;
	...
	int WeightProfile::calc_weight(stmt* s, BufStateTblType* drvMap) {
		SMLog::SMLOG(10, "Entering WeightProfile::calc_weight");	
	...
	void WeightProfile::update() {
		SMLog::SMLOG(10, "Entering WeightProfile::update");	
	...
	void WeightProfile::update_priority(StmtEntry* se) {
		SMLog::SMLOG(10, "Entering WeightProfile::update_priority");	
	...
	void WeightProfile::printStateTable() {
		SMLog::SMLOG(10, "Entering WeightProfile::printStateTable");	
	...
	run_rc WeightGroupSchedule::runState(b_entry state, int &nTuples, bool* output) {
		SMLog::SMLOG(10, "Entering WeightGroupSchedule::runState");	
		...
		switch (state->action) {
			case act_cjump:
				...
				if ((int)(s_rc = (monitor->exeStmt(state->statement, drv))) > 0 || 
					(int) s_rc == s_no_output || (int)s_rc == s_failure) 
				{
					if (querySchdl::verbose) {
...
(int)s_rc == s_no_output || (int)s_rc == s_failure) {
		...
		SMLog::SMLOG(10, "Entering ProfileSchedule::update");	
		...
		SMLog::SMLOG(10, "Entering ProfileSchedule::printStateTable");	
			for (vector<WeightGroupSchedule*>::iterator itr = vSGroupSchedules.begin(); itr != vSGroupSchedules.end(); ++itr) {
				(*itr)->printStateTable();
			}
		}
		...
		SMLog::SMLOG(10, "Entering ScheduleStrategy::ScheduleStrategy");	
			this->drv = drv;
		...
		SMLog::SMLOG(10, "Entering ScheduleStrategy::setDriver");	
			this->drv = drv;
			drv->setStrategy(this);
		...
		SMLog::SMLOG(10, "Entering ScheduleStrategy::update");	
			if (profile == NULL) {
		...
		run_rc WRRGroupSchedule::runNextUnit(int &nTuples) {
			...
			void OCSProfile::printStateTable() {
				SMLog::SMLOG(10, "Entering OCSProfile::printStateTable");	
			...
			void OCSProfile::update() {
				SMLog::SMLOG(10, "Entering OCSProfile::update");	
			...
			StmtEntry* OCSProfile::update_priority(stmt* s) {
				SMLog::SMLOG(10, "Entering OCSProfile::update_priority");	
			...
			void OCSProfile::update_quotas() {
				SMLog::SMLOG(10, "Entering OCSProfile::update_quotas");	
				...
				{
		SMLog::SMLOG(10, "Entering PriorityStrategy::printStateTable");	
				...
				{  
		SMLog::SMLOG(10, "Entering OperatorPriorityStrategy::update_all");	
					if (querySchdl::verbose) {
				...
				{  
//yy		SMLog::SMLOG(10, "Entering OperatorPriorityStrategy::runNextUnit");	
					//if (querySchdl::verbose) {
