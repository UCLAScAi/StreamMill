...
// If set to 1, then even before start-up, all queries will be recompiled.
// If set to 0, then only source code for queries will be generated.
#define COMPILE_QUERIES 1
...
static const char *compile_cmd={"./eslcc %s >& errorsFile"}; 
...
int compiler::compile(const char* queryName, compile_opt opt){
	SMLog::SMLOG(10, "Entering compiler::compile");	
	...
	SMLog::SMLOG(12, "===============================");	
	SMLog::SMLOG(12, "queryName: %s", queryName);	
	SMLog::SMLOG(12, "START==========================");	
	SMLog::SMLOG(12, buf);	
	SMLog::SMLOG(12, "END============================");	
...
exit:
	...
	if (rc==0){  // compilation succeeds
		...
		SMLog::SMLOG(10, "Entering compiler::redirectStdout %s", cmd);	
		cout<<cmd<<endl;
		if (COMPILE_QUERIES) {
		  rc = system(cmd);
		}
	...
	free(errors);
	SMLog::SMLOG(10, ">Exiting compiler::compile");	
	return rc;
}
