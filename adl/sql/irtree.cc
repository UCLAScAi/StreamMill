#include <stdio.h>
#include <assert.h>
#include <sql/nt_sys.h>
#include <sql/ntsql_symtab.h>
#include <sql/ntsql_temp.h>
#include <sql/ntsql_irtree.h>

T_expList T_ExpList(T_exp head, T_expList tail)
{
  T_expList p = (T_expList) ntMalloc (sizeof *p);
...
{
  T_stmList p = (T_stmList) ntMalloc (sizeof *p);
  p->head=head; p->tail=tail;
  return p;
}
...
{
  T_stm p = (T_stm) ntMalloc(sizeof *p);
  p->kind=T_SEQ;
  p->u.SEQ.left=left;
  p->u.SEQ.right=right;
  return p;
...
{
  T_exp p = (T_exp) ntMalloc(sizeof *p);
  p->kind=T_BINOP;
  p->u.BINOP.op=op;
  p->u.BINOP.left=left;
