...
int _adl_statement_9()
{
   ...
   while (rc==0) {
      ...
      if (rc == DB_NOTFOUND) first_entry_8=1;
         ...
         memcpy(insert_7.field_1, "100", 4);
         insert_7.field_1[4]=0;
      } /* if (rc == 0) */
      if (rc ==0 && slide_out == 1) {
         _adl_cursqlcode = 0; /* SUCCESS */ 
         /* INSERT STARTS */
...
int _adl_statement_12()
{
   ...
   while (rc==0) {
      ...
      if (rc == DB_NOTFOUND) first_entry_11=1;
         ...
         memcpy(insert_10.field_1, "6", 2);
         insert_10.field_1[2]=0;
      } /* if (rc == 0) */
      if (rc ==0 && slide_out == 1) {
         _adl_cursqlcode = 0; /* SUCCESS */ 
         /* INSERT STARTS */
...
int _adl_statement_15()
{
   ...
   while (rc==0) {
      ...
      if (rc == DB_NOTFOUND) first_entry_14=1;
         ...
         memcpy(insert_13.field_1, "2", 2);
         insert_13.field_1[2]=0;
      } /* if (rc == 0) */
      if (rc ==0 && slide_out == 1) {
         _adl_cursqlcode = 0; /* SUCCESS */ 
         /* INSERT STARTS */
...
int _adl_statement_18()
{
   ...
   while (rc==0) {
      ...
      if (rc == DB_NOTFOUND) first_entry_17=1;
         ...
         memcpy(insert_16.field_1, "10", 3);
         insert_16.field_1[3]=0;
      } /* if (rc == 0) */
      if (rc ==0 && slide_out == 1) {
         _adl_cursqlcode = 0; /* SUCCESS */ 
         /* INSERT STARTS */
...
int _adl_statement_24()
{
   ...
   while (rc==0) {
      ...
      if (rc == DB_NOTFOUND) first_entry_23=1;
         ...
         memcpy(insert_22.field_1, "0", 2);
         insert_22.field_1[2]=0;
      } /* if (rc == 0) */
      if (rc ==0 && slide_out == 1) {
         _adl_cursqlcode = 0; /* SUCCESS */ 
         /* INSERT STARTS */
...
int _adl_statement_25()
{
   ...
   _adl_cursqlcode = 1 /* ASSUME GUITY */;
   {
      FILE *_adl_load = fopen("tmp.stream", "rt");
      char _adl_load_buf[40960], *tok;
      char loadkeybuf[1], loaddatabuf[9];
      int _adl_line_no=0;
      char bPoint =0;
      if (!_adl_load) {
         printf("can not open file tmp.stream.\n");
         exit(1);
      }
      memset(&key, 0, sizeof(key));
      memset(&data, 0, sizeof(data));
