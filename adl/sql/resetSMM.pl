...
# This script restarts SMM by creating the default user (smm) and
# default aggregates.
...
system(3);
system("./esl_start -p 5455&");
sleep(10);

# Create a new user smm with password smmsmm
send_message("localhost", $sys_port, "AddNewUser smm smm smm\@smm.com smmsmm");
printf("Added user\n");
sleep(3);

send_message("localhost", $sys_port, "makePublic smm smm");
printf("Made her public\n");
sleep(3);
...
send_message("localhost", $sys_port, "AddQueries smm qa\nselect station_id, speed from traffic;");
printf("Added query\n");
sleep(3);

send_message("localhost", $sys_port, "AddQueries smm qb\nselect station_id, sum(speed) over(partition by station_id ROWS 2 preceding) from traffic;");
printf("Added query\n");
sleep(3);

send_message("localhost", $sys_port, "AddStreams smm trainflowers\nstream trainflowers(id int, SL real, SW real, PL real, PW real, isSetosa int) source 'port5552';");
printf("Added stream\n");
sleep(3);

send_message("localhost", $sys_port, "AddStreams smm testflowers\nstream testflowers(id int, SL real, SW real, PL real, PW real, isSetosa int) source 'port5553';");
printf("Added stream\n");
sleep(3);


send_message("localhost", $sys_port, "AddStreams smm trainVert\nstream trainVert(id int, col int, val int, cLbl int);");
printf("Added stream\n");
sleep(3);

send_message("localhost", $sys_port, "AddStreams smm testVert\nstream testVert(id int, col int, val int, cLbl int);");
printf("Added stream\n");
sleep(3);

send_message("localhost", $sys_port, "AddTables smm classStat\ntable classStat(classLbl int, cnt real) memory;");
printf("Added table\n");
sleep(3);

send_message("localhost", $sys_port, "AddTables smm colStat\ntable colStat(col int, val int, classLbl int, cnt real) memory;");
printf("Added table\n");
sleep(3);

send_message("localhost", $sys_port, "AddAggregate smm aggregate dissembleFlowers(id int, v1 Int, v2 Int, v3 Int, v4 Int, classLbl Int):(id int, col Int, val Int, classLbl Int)
{
 initialize: iterate: {
...
send_message("localhost", $sys_port, "AddAggregate smm Aggregate learnClassifier(coli int, vali int, classLbli int, numColsi int):(a int) {
 initialize: iterate: {
...
send_message("localhost", $sys_port, "AddAggregate smm Aggregate evaluateClassifier(idi int, coli int, vali int, numColsi int, classLbli int):(ido int, classLblo int, classo int) {
