/* I am supprised to learn that there are BUGs in the code!  I will
   think again about using GNU code. In hash_ask (), we have to keep
   on scanning the slots even if the current slot is DELETED.
   --Haixun Wang
...
#ifdef  TEST_HASH_INTERNAL
#define START_POWER    (3)	/* power of two: size of new hash table */
#else
#define START_POWER    (10)	/* power of two: size of new hash table */
#endif

/* TRUE if a symbol is in entry @ ptr.  */
#define islive(ptr) (ptr->hash_key && ptr->hash_key!=DELETED)
...
struct hash_entry
{
    /* points to where the symbol string is */
    /*
    ** J.Kong: I modified original hash implementation here.
    ** The original implementation is for symbol-table lookup
    ** where only text strings are possible.
    **
    ** Now it can hold anything with size of "key_size"
    */
    const void *hash_key;
    size_t key_size;
...
static int hash_code PARAMS ((struct hash_control *, const void *,
			      const size_t));
...
hash_new ()
{
    ...
    if ( !(room = malloc(sizeof (struct hash_entry) * ((1 << START_POWER) + 1))) )
      /* +1 for the wall entry */
      fprintf(stderr, "Internal error!: fail to create hash table");
    if ( !(retval = malloc(sizeof (struct hash_control))))
      fprintf(stderr, "Internal error!: fail to create hash table");
...
hash_delete (handle, key, key_size)
struct hash_control *handle;
const void *key;
size_t key_size;
{
    ...
    entry = hash_ask (handle, key, key_size, STAT__WRITE);
    if (hash_found)
    {
	retval = entry->hash_value;
	entry->hash_key = DELETED;
 ...
 */
PTR
hash_replace (handle, key, key_size, value)
struct hash_control *handle;
const void *key;
...
PTR value;
{
	...
	entry->hash_key = key;//Haixun Wang
    }
    else
    {
	retval = NULL;
...
hash_insert (handle, key, key_size, value)
struct hash_control *handle;
const void *key;
const size_t key_size;
PTR value;
 ...
 */
const char *
hash_jam (handle, key, key_size, value)
struct hash_control *handle;
const void *key;
const size_t key_size;
PTR value;
...
struct hash_control *handle;
{
    ...
    if ( !(newwhere = malloc ( (unsigned long) ((temp << (GROW_FACTOR + 1))
					      /* +1 for wall slot */
						* sizeof (struct hash_entry) ))))
      fprintf(stderr, "Internal error!: fail to grow hash table");
    if (newwhere == NULL)
	return "no_room";
	...
	if (((key = oldtrack->hash_key) != NULL) && key != DELETED)
	    if ((retval = hash_jam (handle, key, oldtrack->key_size, oldtrack->hash_value)))
		return retval;
 ...
 *      void myfunct(key, key_size, value)
 *              void * key;           // the symbol's name
 *              size_t key_size;      // the
...
void (*function) ();
{
	...
	{
	    (*function) (handle, entry->hash_key, entry->key_size, entry->hash_value, entry-handle->hash_where);
	}
    }
}

void
applicatee (handle, key, key_size, value, slot)
struct hash_control *handle;
void *key;
size_t key_size;
char *value;
int slot;
{
  //    int p = *(int*)key;
  char *p=key;
    printf ("%c%c%c => %d,%d,%d\n", p[0],p[1],p[2], value, hash_code(handle, key, key_size), slot);
}
...
const size_t key_size;
{
    ...
    entry = hash_ask (handle, key, key_size, STAT__READ);
    if (hash_found)
...
hash_ask (handle, key, key_size, access_type)
struct hash_control *handle;
const void *key;
const size_t key_size;
int access_type;
{
    ...
    struct hash_entry *slot_ought_to_live = NULL;
    struct hash_entry *end_slot;
    int collision;	/* count collisions */
    int strcmps;
    int hcode;

    /* start looking here */
    hcode = hash_code (handle, key, key_size);
    ...
    do {
      ...
      if (s==NULL) {
	if (slot_ought_to_live == NULL) slot_ought_to_live = slot;
	break;
      } else if (s==DELETED) {
	if (slot_ought_to_live == NULL) slot_ought_to_live = slot;
      } else {
	if (key == s  ||
	    (slot->h == (unsigned long) hcode && 
	     memcmp(key, s, key_size)==0)) {
	  strcmps++;		/* not exactly */
	  slot_ought_to_live = slot;
	  hash_found = TRUE;
	  break;
	}
      }
...
#if 0
    while (((s = slot->hash_key) != NULL)) // && s != DELETED)
    {
      if (s == DELETED && slot_ought_to_live == NULL) {
	/* unless we found it else where */
	slot_ought_to_live = slot;
    ...
    if (s == NULL && slot != handle->hash_wall && slot_ought_to_live == NULL) {
      slot_ought_to_live = slot;
    }
    /*
     * slot:                                                      return:
...
main ()
{
    ...
    char *what ();
    int *ip;
    char *a[]={"cbc","cbd","cbe", "bbp","bbf","dbg", "ccd", "cce"};
    char *b[]={"abc","123","345", "abc","123","345", "xxx", "yyy"};
    int i;
    h = (struct hash_control *)hashtable[number] = (char *) hash_new ();

    for (i=0;i<4; i++) 
      hash_insert(h, a[i], 3, b[i]);

    printf("0: current hash-->\n");hash_apply (h, applicatee);
    hash_delete(h, a[2], 3);

    printf("1: current hash-->\n");hash_apply (h, applicatee);

    for (i=4;i<8; i++) 
      hash_insert(h, a[i], 3, b[i]);
    printf("2: current hash-->\n");hash_apply (h, applicatee);

    hash_delete(h, a[3], 3);
    hash_delete(h, a[5], 3);
    printf("3: current hash-->\n");hash_apply (h, applicatee);
    hash_insert(h, a[2], 3, b[2]);
    printf("4: current hash-->\n");hash_apply (h, applicatee);
    hash_insert(h, a[5], 3, b[5]);
    printf("5: current hash-->\n");hash_apply (h, applicatee);
    hash_insert(h, a[3], 3, b[3]);
    printf("6: current hash-->\n");hash_apply (h, applicatee);
    for (i=0;i<8;i++) {
      char *p = hash_find(h, a[i], 3);
      printf("find %s get %s\n", a[i], p);
    }
	...
	{
	    int num;
	    name = what("symbol/numeric");
	    if(isdigit(name[0]))
	    {
		num = atoi(name);
		p = hash_find(h, (char *)&num, sizeof(int));
		printf("value of \"%d\" is \"%s\"\n", num, p? p:"NOT-PRESENT");
	    }
	    else
	    {
		p = hash_find(h, name, strlen(name));
	...
	case 'i':
	{
	    ...
	    if(isdigit(name[0]))
	    {
		numerics[num_idx] = atoi(name);
		p = (char *)hash_insert (h, (char *)&numerics[num_idx],
					 sizeof(int), value);
		if(p)
		    printf("symbol=\"%d\"  value=\"%s\"  error=%s\n",
			   numerics[num_idx], value, p);
		num_idx++;
	    }
	    else
	    {
		p = (char *)hash_insert (h, name, strlen(name), value);
		if(p)
	...
	case 'j':
	{
	    ...
	    {
		numerics[num_idx] = atoi(name);
		p = (char *)hash_jam(h, (char *)&numerics[num_idx],
				     sizeof(int), value);
		if(p)
		    printf("symbol=\"%d\"  value=\"%s\"  error=%s\n",
			   numerics[num_idx], value, p);
	    ...
	    {
		p = (char *)hash_jam(h, name, strlen(name), value);
		if(p)
		    printf ("symbol=\"%s\"  value=\"%s\"  error=%s\n",
			    name, value, p);
	    }
	...
	case 'n':
	    h = (struct hash_control *)hashtable[number] =
		(char *) hash_new ();
	    break;
	case 'q':
	    exit (EXIT_SUCCESS);
	case 'r':
	{
	    ...
	    {
		numerics[num_idx] = atoi(name);
		p = (char *)hash_replace(h, (char *)&numerics[num_idx],
					 sizeof(int), value);
		if(p)
		    printf("symbol=\"%d\"  value=\"%s\"  error=%s\n",
			   numerics[num_idx], value, p);
		...
		p = (char *)hash_replace(h, name, strlen(name), value);
	...
	case 'x':
	{
	    ...
	    {
		num = atoi(name);
		p = hash_delete(h, (char *)&num, sizeof(int));
	    }
	    else
		p = hash_delete (h, name, strlen(name));
