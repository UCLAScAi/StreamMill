...
public class Exec {
  ...
  public static boolean execWaitPrint(String command) {
  ...
  private static boolean exec(String command,
    ...
    if (verbose) {
      newPrintSeparator();
      printSeparator();
      StreamMill.getResult().append("Executing '" + command + "'.");
    }
    try {
	...
	BufferedReader commandResult = new BufferedReader(new InputStreamReader(buffer));
        ...
        try {
			...
			while ((s = commandResult.readLine()) != null) {
				printSeparator();
				StreamMill.getResult().append("Output: " + s);
				rowCount++;
			}
			if (s == null) {
				printSeparator();
				StreamMill.getResult().append("Successfully Executing: " + command);
			}
  ...
  private static void printError(String command,
                                 Exception e) {
    	printSeparator();
    	StreamMill.getResult().append("Error doing exec(" + command + "): ");
	
	if (e.getMessage() != null) {
		printSeparator();
    StreamMill.getResult().append(e.getMessage());
	}	
	 printSeparator();
   StreamMill.getResult().append("Did you specify the full " + "pathname?");
  }

  private static void printError(String command) {
	  printSeparator();
    StreamMill.getResult().append("Error executing '" + command + "'.");
  }
    
  //----------------------------------------------------

  private static void newPrintSeparator() {
   
    if (StreamMill.getCleanPane() == true){
      StreamMill.getResult().append("==============================================");
    }
    else {
    	printSeparator();
    	printSeparator();
